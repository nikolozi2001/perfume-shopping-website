...
export const useRouteMetadata = () => {
  ...
  useEffect(() => {
    ...
    const currentMatch = matches.filter((match) => match.handle).pop();
...
export const useRouteGuard = (requiredPermissions = []) => {
  ...
  const hasAccess = requiredPermissions.length === 0 || checkPermissions();
...
export const preloadRoute = async (routePath) => {
  try {
    ...
    const routeMap = {
      ...
      "/fragrance": () =>
        import("../assets/components/Pages/Fragrance/Fragrance.jsx"),
      "/fragrance_sets": () =>
        import(
          "../assets/components/Pages/Gift_Sets/Fragrance_Sets/FragranceSets.jsx"
        ),
      "/contacts": () =>
        import("../assets/components/Pages/Contacts/Contacts.jsx"),
    ...
    // Failed to preload route - could be logged to analytics instead
  }
};
